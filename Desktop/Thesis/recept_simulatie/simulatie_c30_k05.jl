...
start = time()
G = creeerToyExample(0.5,ncomp=30);
...
for i = 1:3
    ...
    estimator = MultilevelEstimators.Estimator(ML(), MC(), level_levensduur, distributions, name="system30_$(i)_k05_MLMC", max_index_set_param=length(Levels)-1,nb_of_warm_up_samples = 100);#,robustify_bias_estimate = false);
    h1 = run(estimator, tol);
    println("finished with estimating");
    estimator = MultilevelEstimators.Estimator(ML(), QMC(), QMC_ML_levensduur, distributions, name="system30_$(i)_k05_MLQMC", max_index_set_param=length(Levels)-1,nb_of_warm_up_samples = 100);#,robustify_bias_estimate = false);
    h2 = run(estimator, tol);
    println("finished with estimating");
end
systeem = Dict(:systeem => G,:levels => Levels, :lijst => T,:invloed => varT,:setup_time => elapsed)
@save "system30_k05_system.jld2" systeem
