...
p = plot()
nsim=3
namefig="system30_k1/system30_k1.png"
...
γ = []
for k = 1:nsim
    @load("system30_k1/system30_k1_$(k)_MLQMC.jld2",history)
    for i = 1:length(history.data)
        for lev = 0:length(history.data[i][:T])-1
    ...
    @load("system30_k1/system30_k1_$(k)_MLMC.jld2",history)
    for i = 1:length(history.data)
...
plot!(tolerance,linr,lw=12,xaxis=:log,yaxis=:log ,color=:green,label = "MLQMC",linestyle=:dot,xlabel="tolerantie",ylabel="runtime(s)",size=(1600,1100),legendfontsize=28,guidefontsize=36,tickfontsize=20,margin=18mm)
